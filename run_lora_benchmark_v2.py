...
def run_benchmark(args):
    ...
    # Check if a fixed seed is provided
    ...
    for idx, raw_prompt in enumerate(prompts, start=1):
        ...
        if fixed_seed is not None:
            prompt_seed = fixed_seed
        else:
            # Generate a consistent seed for this prompt based on its content
            prompt_seed = int(hashlib.md5(prompt.encode()).hexdigest(), 16) % (2**32)
